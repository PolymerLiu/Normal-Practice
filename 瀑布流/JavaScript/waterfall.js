window.onload=function(){
	...
	window.onscroll = function(){   //检测页面是否滚动
		if (checkScrollSlide()) {
			var oParent=document.getElementById('main');
			for (var i = 0; i < dataInt.data.length; i++) {//dataInt是对象，并无length属性，dataInt.data属性才有length属性
			...
			waterfall('main','box');  ///再次调用waterfall函数使新加载的图片以瀑布流的方式排布
		}
	}
}
...
function waterfall(parent,box){
	...
	for (var i = 0; i < oBoxs.length; i++) {
		if (i<cols) {
			hArr.push(oBoxs[i].offsetHeight);  //获取的是第一行图片的高度
		}else{
			...
			hArr[index]+=oBoxs[i].offsetHeight;  //获取的是每一列图片的高度
		}
	}
}
...
function checkScrollSlide(){
	...
	var height=document.body.clientHeight || document.documentElement.clientHeight;  //窗口高度   //获取混杂模式 || 标准模式 高度
